"""
TileLang算子性能基准测试模板 - your_op_name

该文件提供了TileLang算子性能测试的基础模板。
请根据实际算子需求修改以下内容：
...
def get_configs():
    ...
    configs = []
    for bm, bn, bk, ns, t, er in itertools.product(
...
def run_autotune(M: int, N: int, K: int, topk: int = 20):
    """运行自动调优"""

    def kernel_fn(block_M=None, block_N=None, block_K=None, num_stages=None, threads=None):
        return matmul_with_config(M, N, K, block_M, block_N, block_K, num_stages, threads)
...
def main():
    ...
    if args.autotune:
        ...
        best_config, kernel = run_autotune(M, N, K)
